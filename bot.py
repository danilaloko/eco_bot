...
class EcoBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.db.is_user_registered(user_id):
            ...
            if not user_data[4] or not user_data[5]:  # first_name и last_name
                ...
                # Сбрасываем флаг регистрации и начинаем заново
                with sqlite3.connect(self.db.db_path) as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE users SET registration_completed = FALSE WHERE user_id = ?', (user_id,))
                    conn.commit()
                
                # Переходим к процессу регистрации
    ...
    async def _send_game_rules(self, update: Update):
        ...
        # Путь к PDF файлу
        pdf_path = "docs/Положение_игра_Заповедные_каникулы_Полевой_сезон_2025.pdf"
        
        # Формируем текст сообщения
        text = "📋 **Положение об игре**\n\n"
        ...
        if self._should_show_july_21_message():
            text += (
                "🎯 **ВАЖНАЯ ИНФОРМАЦИЯ!**\n"
                "📅 **Первое задание будет открыто 21 июля 2025 года**\n\n"
                "Ознакомьтесь с правилами и будьте готовы к участию!\n\n"
            )
        ...
        try:
            if os.path.exists(pdf_path):
                with open(pdf_path, 'rb') as pdf_file:
                    await update.message.reply_document(
                        ...
                        caption=text,
                        parse_mode='Markdown',
                        filename="Положение_игра_Заповедные_каникулы_Полевой_сезон_2025.pdf"
                    )
                ...
                text = (
                    "📋 **Положение об игре**\n\n"
                    "⚠️ PDF файл с положением временно недоступен. "
            ...
            logger.error(f"Ошибка при отправке PDF файла: {e}")
            
            # В случае ошибки отправляем только текст
            text = (
                "📋 **Положение об игре**\n\n"
                "⚠️ Произошла ошибка при загрузке PDF файла. "
                "Обратитесь в поддержку для получения актуального документа.\n\n"
            ) + text
    ...
    async def _send_instructions(self, update: Update):
        ...
        if self._should_show_july_21_message():
            text += (
                "🎯 **ВАЖНАЯ ИНФОРМАЦИЯ!**\n"
                "📅 **Первое задание будет открыто 21 июля 2025 года**\n\n"
                "Изучите инструкцию и будьте готовы к участию!\n\n"
            )
